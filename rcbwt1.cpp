...
//   [ n bytes BWT ][ 4 bytes LE primary index pi ]
//
...
// File layout: [ n BWT bytes ][ 4 bytes LE pi ]  (pi at end, matching the
// BWT.cpp / unBWT.cpp reference pair and the current rcbwt.cpp convention).
                      ...
                      const uint8_t* bwt, size_t n)
{
    ...
    if (fwrite(tail, 1, 4, f) != 4) { perror("fwrite"); fclose(f); return 0; }
    fclose(f);
    return 1;
}
...
int main(int argc, char** argv) {
    ...
    (void)K_MAX_LEVELS; // reserved for future recursive key extension
